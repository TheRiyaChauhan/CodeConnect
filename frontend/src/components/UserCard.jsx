...
const handleSendRequest = async (status, userId) => {
  ...
  return (
    <div className="card bg-base-100 w-85  rounded-xl overflow-hidden shadow-2xl my-4 h-[77vh]">
      {/* Picture */}
      ...
      <div className="card-body px-6 py-2 text-center space-y-2">
        ...
        <div className="card-actions justify-between pt-4 mb-3">
          <button className="btn btn-error btn-m flex-1 text-[17px] "  onClick={() => handleSendRequest("ignored", user._id)}>
            Ignored
          </button>
          <button className="btn btn-success btn-m flex-1 text-[17px] text-green-950" onClick={() => handleSendRequest("interested", user._id)}>
            Interested
          </button>
        </div>
      </div>
